...
extern S32 Alea(S32 i_iMin, S32 i_iMax);

BaseObject_ZHdl ClassNameResManager_Z::GetObjectInClass(const S32 i_ClassID, const S32 i_EnumID) {
    const S32Hash_Z* l_Result;
    if (this != *(ClassNameResManager_Z**)(&gData.m_Pad_0x14[0x28])) { // If it's not AnimMgr
        const S32Hash_Z l_HashElt(i_EnumID);
        l_Result = m_ClassResHA[i_ClassID].m_ResourceHT.Search(l_HashElt);
    }
    else {
        if (!m_ClassResHA[i_ClassID].m_ResourceHT.GetNbElem()) {
            return HANDLE_NULL;
        }

        S32 l_Id = Alea(0, m_ClassResHA[i_ClassID].m_ResourceHT.GetNbElem() - 1);

        m_ClassResHA[i_ClassID].m_ResourceHT.InitScan();

        S32Hash_Z* l_Temp = m_ClassResHA[i_ClassID].m_ResourceHT.NextScan();
        S32 i = 0;
        while (l_Temp && l_Id > i) {
            l_Temp = m_ClassResHA[i_ClassID].m_ResourceHT.NextScan();
            i++;
        }
        l_Result = l_Temp;
    }
